...
raw_Data_ex3 <- httr::GET(url = 
    paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", 
           UNITCODES, "&startMonth=", STARTMONTH, "&startYear=", STARTYEAR, 
           "&endMonth=", ENDMONTH, "&endYear=", ENDYEAR))

# convert content to text
extracted_data_ex3 <- httr::content(raw_Data_ex3, as = "text", encoding = "UTF-8") 

# parse text from JSON to data frame
final_data_ex3 <- jsonlite::fromJSON(extracted_data_ex3)

return(final_data_ex3)
...
```{r}
pull_vdROMO <- unit_visitation(UNITCODES = "ROMO" , STARTMONTH = "01" , 
                             STARTYEAR =  1990, ENDMONTH = "12", ENDYEAR = 2021)

pull_vdEVER <- unit_visitation(UNITCODES = "EVER" , STARTMONTH = "01" , 
                             STARTYEAR =  1990, ENDMONTH = "12", ENDYEAR = 2021)

pull_vdTHRO <- unit_visitation(UNITCODES = "THRO" , STARTMONTH = "01" , 
                             STARTYEAR =  1990, ENDMONTH = "12", ENDYEAR = 2021)
```
...
```{r}
pull_vdROMO <- unit_visitation(UNITCODES = "ROMO" , STARTYEAR =  1990, ENDYEAR = 2021)

pull_vdEVER <- unit_visitation(UNITCODES = "EVER" , STARTYEAR =  1990, ENDYEAR = 2021)

pull_vdTHRO <- unit_visitation(UNITCODES = "THRO" , STARTYEAR =  1990, ENDYEAR = 2021)
```
...
outputpark_floop <- vector("list", length = length(parkcodes))
```
...
for(i in 1:length(parkcodes)){
  ...
  outputpark_floop[[i]] <-
    unit_visitation(UNITCODES = parkcodes[i], STARTYEAR = 1980, ENDYEAR = 2021)
}
...
```{r}
summary(outputpark_floop)
  multi_parks <- dplyr::bind_rows(outputpark_floop)
```
...
```{r}
identical(outputpark_floop, outputpark_map)
multi_parks_map <- bind_rows(outputpark_map)  
```
